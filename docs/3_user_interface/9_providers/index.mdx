...
all timing exists as this can be highly dependent on the service provider.

Cert Warden used to query dns records in an attempt to determine when to move
forward. This method was unreliable. Instead, starting with v0.28.0 Cert Warden only
uses a specified wait time after it provisions the needed records. i.e., After
Cert Warden confirms the record has actually been created, it will then wait the
specified amount of time before signaling to the ACME provider to validate the
record.

* `Post-Provision Wait` is the amount of time Cert Warden will wait after it confirms
  the needed resource has been created, before signaling to the ACME server that it
  should proceed with validation.

You should set the minimum time needed for your records to reliably propagate.
For most providers this is 3 to 5 minutes (180 to 300 seconds).
